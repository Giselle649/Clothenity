...
useEffect(() => {
  ...
  const formOnSubmitHandler = async (event) => {
    ...
    try {
      ...
      if(isItEdit) {
        await axios.put(`http://localhost:3000/products/detail/${id}`, dataToSend, {headers: {
              Authorization: `Bearer ${token}`,
            }})
      } else {
  ...
  if(error) return <p>Error fetching, please try again</p>
   ...
   {/* Halaman Create Entitas Utama */}
     <section>
       <form className="rounded-xl bg-gray-100 px-16 py-16 shadow-2xl max-sm:px-8" onSubmit={formOnSubmitHandler}> 
         ...
         <div className="sm:col-span-4">
             ...
             <div className="mt-2">
                 <div className="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                     ...
                     <input type="text" name="price" id="price"  className="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-350 focus:ring-0 sm:text-sm sm:leading-6" placeholder="350.000" value={eachProduct.price}  onChange={(e) => {setEachProduct({...eachProduct, price: e.target.value})}}/>
                 </div>
             </div>
         </div>
